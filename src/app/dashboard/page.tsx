...
export default function DashboardPage() {
  ...
  const [dataSource] = useState<DataSource>("auto");
  const [isLoading, setIsLoading] = useState<boolean>(false);
  const [showConnect, setShowConnect] = useState(false);
  const { refreshSubscription } = useSubscription();
  // BankID connection state omitted in UI; keep for future if needed
  const [plaidConnected, setPlaidConnected] = useState<boolean | null>(null);
  const [stripeConnected, setStripeConnected] = useState<boolean | null>(null);
  const [stripeData, setStripeData] = useState<{ id?: string; account_type?: string; connected_at?: string } | null>(null);
  ...
  const fetchData = useCallback(
    async (source: DataSource = dataSource) => {
      ...
      try {
        ...
        const [, plaidStatus, stripeStatus] = await Promise.all([
          bankidCheck,
          plaidCheck,
          stripeCheck,
        ]);
        // We no longer render BankID status dot; keep value local if needed later
        setPlaidConnected(plaidStatus);
        setStripeConnected(stripeStatus);

        if (source === "auto") {
          // Prefer Plaid first; fall back to BankID
          const pr = await authedFetch("/api/plaid/accounts").catch(
          ...
          if (pr && pr.ok) {
            ...
            const prt = await authedFetch("/api/plaid/transactions").catch(
              () => new Response(null, { status: 500 })
            );
            if (prt && prt.ok) {
            ...
            console.log("[Dashboard] Plaid failed, trying BankID...");
  ...
  useEffect(() => {
    if (accounts.length === 0) {
      if (selectedAccountIndex !== 0) setSelectedAccountIndex(0);
      return;
    }
    if (selectedAccountIndex >= accounts.length) {
      setSelectedAccountIndex(0);
    }
  }, [accounts.length, selectedAccountIndex]);

  // Listen for Stripe status change events from ConnectStripe component
  useEffect(() => {
    const onStripeStatusChanged = () => {
      fetchData();
    };
    window.addEventListener("stripe-status-changed", onStripeStatusChanged as EventListener);
    const onPlaidStatusChanged = () => {
      fetchData();
    };
    window.addEventListener("plaid-status-changed", onPlaidStatusChanged as EventListener);
    return () => {
      window.removeEventListener("stripe-status-changed", onStripeStatusChanged as EventListener);
      window.removeEventListener("plaid-status-changed", onPlaidStatusChanged as EventListener);
    };
  }, [fetchData]);
  ...
  useEffect(() => {
    ...
    if (urlParams.get("success") === "true") {
      console.log(
        "[Dashboard] Stripe checkout success detected, refreshing subscription..."
      );
    ...
    if (urlParams.get("stripe_connected") === "true") {
      console.log(
        "[Dashboard] Stripe Connect success detected, refreshing connection status..."
      );
      ...
      <div
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-3">
                ...
                <button
                  ...
                  className="group inline-flex items-center gap-2 px-3 py-1.5 text-[13px] font-medium text-white rounded-lg bg-gray-900/95 backdrop-blur-sm border border-gray-900/80 shadow-sm transition-all duration-200 hover:bg-gray-900 hover:-translate-y-0.5 active:translate-y-0 cursor-pointer hover:shadow-md hover:shadow-gray-900/50">
        ...
        <main className="max-w-7xl mx-auto px-6 py-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  ...
                  <div className="bg-gray-50 rounded-lg p-4">
                    ...
                    <div className="text-sm text-gray-900 capitalize">
                      {stripeData.account_type}
                    </div>
                  </div>
                  <div className="bg-gray-50 rounded-lg p-4">
                    <div className="text-sm text-gray-600 mb-1">Connected</div>
                      <div className="text-sm text-gray-900">
                        {stripeData?.connected_at ? new Date(stripeData.connected_at as string).toLocaleDateString() : "—"}
                      </div>
                  </div>
                </div>
              </div>
          ...
          <div className="space-y-8">
            ...
            {!MVP_MODE && <UsageLimitsChecker />}

            {/* AI Insights Section */}
            {!MVP_MODE && subs.length > 0 && (
            ...
            {subs.length > 0 && (
              <section className="mb-8">
                <div className="bg-linear-to-br from-card-bg-start-light to-card-bg-end-light border border-border-light rounded-2xl p-8 shadow-sm">
                              ...
                              <div
                                ...
                                <div className="flex items-center gap-3 flex-1">
                                  ...
                                  <div className="flex-1">
                                    ...
                                    <div className="w-full bg-background-light/50 rounded-full h-2 mt-1">
                                      <div
                                          className="bg-linear-to-r from-primary to-primary/70 h-2 rounded-full transition-all duration-500"
                                        style={{
                                          width: `${percentage}%`,
                                        }}></div>
                                    </div>
                                ...
                                <div className="text-right ml-4">
                                  <p className="font-semibold text-foreground-black text-sm">
                                    {sub.monthlyAmount.toLocaleString("sv-SE", {
                                      style: "currency",
                                      currency: "SEK",
                                    })}
                                  </p>
            ...
            {accounts.length > 0 && (
              <section>
                <div className="flex items-center justify-between mb-6">
                  ...
                  <div className="flex items-center gap-3">
                    <label className="text-sm text-gray-600" htmlFor="account-select">Select</label>
                    <select
                      id="account-select"
                      value={selectedAccountIndex}
                      onChange={(e) => setSelectedAccountIndex(parseInt(e.target.value, 10))}
                      className="text-sm border border-gray-300 text-gray-600 rounded-md px-2 py-1 bg-white focus:outline-none cursor-pointer hover:border-gray-400 hover:shadow-sm"
                    >
                      {accounts.map((account: Account, i: number) => (
                        <option key={i} value={i}>
                          {(account.name || account.official_name || "Account") + (account.subtype ? ` • ${account.subtype}` : account.type ? ` • ${account.type}` : "")}
                        </option>
                      ))}
                    </select>
                        ...
                        <div className="text-right">
                          <p className="text-lg font-semibold text-gray-900">
                            {(() => {
                              ...
                              return `${balance.toLocaleString("sv-SE", { style: "currency", currency })}`;
                            })()}
                          </p>
            ...
            {/* Empty State - show only when neither Plaid nor Stripe is connected */}
            {!plaidConnected && !stripeConnected && (
              <div className="max-w-4xl mx-auto">
                <div className="text-center py-16">
                  ...
                  <p className="text-lg text-gray-600 mb-8 max-w-2xl mx-auto">
                    Connect your bank account to automatically detect recurring
                    subscriptions and take control of your spending.
                  </p>
                  ...
                  <div className="flex flex-col sm:flex-row gap-4 justify-center mb-12">
                    ...
                    {/* Stripe secondary CTA */}
                    <button
                      onClick={() => setShowConnect(true)}
                      className="inline-flex items-center justify-center gap-3 px-8 py-4 bg-white border-2 border-gray-300 text-gray-900 rounded-lg font-semibold hover:bg-gray-50 hover:border-gray-400 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-purple-500 transition-colors">
                ...
                <div className="bg-gray-50 border border-gray-200 rounded-lg p-8 text-center">
                  ...
                  <p className="text-sm text-gray-600">
                    Trusted by thousands of users to manage their subscriptions
                    securely
                  </p>
                </div>
              </div>
